...
class LResNetXEIR(nn.Module):
    def __init__(self, block, layers, zero_init_residual=True, num_classes=10):
        ...
        self.loss = CosFace_EPL(512, num_classes)
        print(self.loss)
    ...
    def forward(self, x, t=None, epoch=None):
        ...
        return self.loss(feat, t, epoch)

# add
def LResNet18EIR(**kwargs):
    model = LResNetXEIR(IRBlock, [2, 2, 2, 2], **kwargs)
    return model

def LResNet34EIR(**kwargs):
    model = LResNetXEIR(IRBlock, [3, 4, 6, 3], **kwargs)
    return model
